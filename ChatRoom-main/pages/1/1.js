...
Page({
    ...
    like:function(e){
        ...
        var that=this;
        const _ = db.command
        db.collection('CET').doc(e.currentTarget.dataset.id).update({
            data: {
    ...
    formSubmit:function(e){
        ...
        that.setData({
         ...
         db.collection('chat_user').where({
            ...
            console.log(app.globalData.userInfo.account_id)   
          })
        ...
        wx.cloud.callFunction({
            ...
            success(res){
                ...
                if(res.result.errCode==0){
                    db.collection("CET").add({//将信息存入数据库
                        data:{
                            random:app.globalData.userInfo.account_id,//用户名称
                            time:time,//时间
    ...
    onLoad: function (options) {
        db.collection("CET").get({//从数据库提取信息
            success:res=>{//成功
